...
*guide* curve along the vessel on each angio, then pins *MultiReg points*
...
class _CoregPane:
    ...
    def __init__(self, index: int, owner: "CoregWindow"):
        ...
        # Reverse the play/scroll order (distal↔proximal) AFTER entering MultiReg,
        ...
        self.include_cb.setToolTip(
            t("While adding a MultiReg point, include this IVUS's current frame"))
    ...
    def load(self, series: Series | None, plane_index: int = 0,
        ...
        *start_frame* opens on the frame the source pane was showing (so an
        ...
        # Angio panes carry the MultiReg overlay; IVUS panes do not but get
        ...
        # IVUS panes scrub freely (they drive MultiReg); an ANGIO pane is a still
    ...
    def _toggle_reverse(self) -> None:
        ...
        the physically-shown frame, mirrors every MultiReg-point frame stored for
        ...
        # Mirror this pane's stored MultiReg-point frames so each point still
    ...
    def _on_slider_pressed(self) -> None:
        ...
        # MultiReg point → drop any landmark selected for modify.
        if self.is_ivus:
...
class CoregWindow(QMainWindow):
    ...
    def __init__(self, pane_specs, parent=None):
        ...
        self._placing: bool = False      # adding/modifying a MultiReg point
        self._pending_fracs: dict[int, float] = {}   # xa_idx -> provisional s
        self._selected_lm: int = -1      # MultiReg point selected (blue) / modify
    ...
    def _build_panel(self) -> QWidget:
        ...
        # MultiReg points section.
        self._cbox = QGroupBox(t("MultiReg points"))
        cv = QVBoxLayout(self._cbox)
        # Mode toggle: white = click to enter create/modify (goes blue); blue
        # = click to commit (確定). Modifies the selected MultiReg point, or
        # creates a new one when none is selected.
        self._add_btn = QPushButton(t("Add / edit / commit MultiReg point"))
        self._add_btn.clicked.connect(self._on_mode_button)
        cv.addWidget(self._add_btn)
        self._sort_btn = QPushButton(t("Sort MultiReg points"))
        ...
        self._clear_all_btn = QPushButton(t("Delete all MultiReg points"))
        self._clear_all_btn.clicked.connect(self._clear_all_landmarks)
        cv.addWidget(self._clear_all_btn)
        # Persist MultiReg points + guide traces to a file, and reload them
        # (also imports a legacy MultiSync .txt as IVUS-only sync points).
        io_row = QHBoxLayout()
        io_row.setContentsMargins(0, 0, 0, 0)
        self._save_btn = QPushButton(t("Save MultiReg…"))
        self._save_btn.setToolTip(
            t("Save the MultiReg points and guide traces to a file"))
        self._save_btn.clicked.connect(self._save_coreg)
        io_row.addWidget(self._save_btn)
        self._load_btn = QPushButton(t("Load MultiReg…"))
        self._load_btn.setToolTip(
            t("Load MultiReg points / guide traces from a file "
        ...
        # Commit a MultiReg point with NO XA position — for landmarks that are
        ...
        self._exclude_xa_cb.setToolTip(
            t("Don't set a position on XA; use this MultiReg point only for "
              "IVUS sync"))
        ...
        # One row per MultiReg point: [MultiReg-N (jump)] [✕ (delete)].
    ...
    def retranslate_ui(self) -> None:
        ...
        self._cbox.setTitle(t("MultiReg points"))
        self._sbox.setTitle(t("Marker display"))
        self._add_btn.setText(t("Add / edit / commit MultiReg point"))
        self._sort_btn.setText(t("Sort MultiReg points"))
        self._sort_btn.setToolTip(t("Sort by the active IVUS frame order"))
        self._clear_all_btn.setText(t("Delete all MultiReg points"))
        self._save_btn.setText(t("Save MultiReg…"))
        self._save_btn.setToolTip(
            t("Save the MultiReg points and guide traces to a file"))
        self._load_btn.setText(t("Load MultiReg…"))
        self._load_btn.setToolTip(
            t("Load MultiReg points / guide traces from a file "
              "(or import a MultiSync .txt)"))
    ...
    def _build_global_seek(self) -> QWidget:
        ...
        radio. Hidden with <2 IVUS; greyed until a MultiReg point links them."""
        w = QWidget()
        row = QHBoxLayout(w)
    ...
    def _refresh_global_seek(self):
        """Show the bar with ≥2 IVUS; enable + (re)range it once a MultiReg point
    ...
    def _add_guide_row(self, pane: _CoregPane) -> None:
        ...
        # marks shown on each MultiReg-point row. This pane is already appended.
    ...
    def _start_placing(self) -> None:
        if self._active_ivus < 0:
            QMessageBox.information(self, "MultiReg", t("There is no IVUS."))
            return
        ...
        if not has_angio:
            ...
            self._status.setText(t(
                "Editing MultiReg-{n}: move the circle on the angio guide to "
                "the target position, then press the button again to commit.",
                n=self._selected_lm + 1))
        else:
            self._status.setText(t(
                "New MultiReg point: on an angio where it can be identified, "
    ...
    def _on_slider_moved(self, pane_idx: int, s: float) -> None:
        # Provisional anchor for this angio while placing a MultiReg point.
        if self._placing:
    ...
    def _commit_placing(self) -> None:
        ...
        if 0 <= self._selected_lm < len(self._landmarks):
            ...
            if not frames:
                QMessageBox.information(
                    self, "MultiReg", t("Select at least one IVUS to include."))
                return
    ...
    def _landmark_info(self, lm) -> str:
        """Two-line summary shown right of the MultiReg button: line 1 = each
    ...
    def _refresh_list(self) -> None:
        ...
        for n, lm in enumerate(self._landmarks, 1):
            ...
            x.setFixedWidth(26)
            x.setToolTip(t("Delete this MultiReg point"))
            ...
            if (n - 1) == self._renaming:
                ...
                ed.setFocus()
                continue
            b = QPushButton(lm.get("name") or f"MultiReg-{n}")   # jump / rename
    ...
    def _goto_landmark(self, idx: int) -> None:
        """Clicking MultiReg-N selects it (blue) and jumps every IVUS that
    ...
    def _rename_landmark(self, idx: int) -> None:
        """Right-click a MultiReg button → edit its short name (≤10 chars) INLINE,
    ...
    def _sort_landmarks(self) -> None:
        """Order MultiReg points along the active IVUS's pull-back (by its
    ...
    def _clear_all_landmarks(self) -> None:
        ...
        if QMessageBox.question(
                self, "MultiReg", t("Delete all MultiReg points?")
    ...
    def _save_coreg(self) -> None:
        """Write the MultiReg points + guide traces to a JSON file. The guide
        ...
        if not self._landmarks and not any(
            ...
            QMessageBox.information(
                self, "MultiReg", t("Nothing to save yet."))
            return
        path, _ = QFileDialog.getSaveFileName(
            self, t("Save MultiReg"), "", t("MultiReg file (*.cosync.json)"))
        if not path:
            return
        ...
        try:
            ...
            QMessageBox.warning(self, t("Save MultiReg"), str(exc))
            return
        self._status.setText(
            t("Saved {n} MultiReg point(s).", n=len(self._landmarks)))

    def _load_coreg(self) -> None:
        """Load MultiReg points + guide traces from a native .cosync.json, or
        import a legacy MultiSync .txt as IVUS-only sync points."""
        path, _ = QFileDialog.getOpenFileName(
            self, t("Load MultiReg"), "",
            t("MultiReg / MultiSync (*.json *.txt);;All files (*)"))
        if not path:
            return
        try:
            ...
            QMessageBox.warning(self, t("Load MultiReg"), str(exc))
            return
        ...
        if landmarks is None:
            QMessageBox.warning(
                self, t("Load MultiReg"), t("Unrecognised MultiReg file."))
            return
    ...
    def _parse_coreg_json(self, data):
        ...
        or (None, None) if the file is not a MultiReg file."""
        # Accept the current format and the pre-rename "MDV-CoReg" files.
        if (not isinstance(data, dict)
                or data.get("format") not in ("MDV-MultiReg", "MDV-CoSync",
                                               "MDV-CoReg")):
            return None, None
        lms = []
    ...
    def _parse_multisync_txt(self, text: str):
        ...
        per-slot frame/rotation becomes an IVUS-only MultiReg landmark. Slot k
    ...
    def _apply_loaded(self, landmarks, guides) -> None:
        """Replace the current MultiReg points + guides with loaded ones and
        ...
        self._drive_markers()
        self._status.setText(
            t("Loaded {n} MultiReg point(s).", n=len(self._landmarks)))
    ...
    def _export_mp4(self) -> None:
        ...
        if self._active_ivus < 0:
            QMessageBox.information(
                self, "MultiReg", t("Select a master IVUS first."))
            return
        master = self.panes[self._active_ivus]
        if not master.is_ivus or master.total < 1:
            QMessageBox.information(
                self, "MultiReg", t("The master IVUS has no frames."))
            return
        ...
        dlg_cfg = ExportDialog(
            ...
            title_override=t("Export MultiReg MP4"), parent=self)
        if dlg_cfg.exec() != dlg_cfg.DialogCode.Accepted:
            return
        cfg = dlg_cfg.result_settings()

        path, _ = QFileDialog.getSaveFileName(
            self, t("Export MultiReg MP4"), "", t("MP4 video (*.mp4)"))
        if not path:
            return
        ...
        prog = QProgressDialog(
            t("Exporting MultiReg MP4…"), t("Cancel"), 0, total, self)
    ...
    def _update_coreg_controls_enabled(self) -> None:
        """Trace and MultiReg-point editing are mutually exclusive: while a
        guide trace is active the MultiReg-point controls are disabled. During
    ...
    def _on_pane_reversed(self, pane) -> None:
        """A pane flipped its play-order (its stored MultiReg-point frames were
    ...
    def _on_ivus_scrub(self, pane_idx: int) -> None:
        ...
        MultiReg points exist) via the frames they share on the landmarks, then
    ...
    def _apply_rotation(self, pane) -> None:
        ...
        the MultiReg landmarks at its current frame (按分). No-op when no
    ...
    def _interp_frac(self, pane_idx: int):
        ...
        a new MultiReg point's marker so it starts on the interpolated spot."""
        R = self._active_ivus
        if R < 0:
            return None
