...
class LVSurface:
    ...
    # ---------------------------------------------------------------- meshing
    def to_mesh(self):
        """Triangulated closed surface (vertices (V,3), faces (F,3) int) — for
        display and for VTK-stencil voxelisation in the viewer. Rings lofted
        with quad strips; apex fanned to ring 0, base fanned to the last ring."""
        ...
        verts = np.concatenate([ring_pts, apex, base], 0)
        apex_i = k * nth
        base_i = k * nth + 1
        faces = []
        ...
        for j in range(nth):                       # apex cap
            jn = (j + 1) % nth
            faces.append([apex_i, jn, j])
        base0 = (k - 1) * nth
        for j in range(nth):                       # base cap (flat)
            jn = (j + 1) % nth
