...
class _PaneCanvas(QVTKRenderWindowInteractor):
    ...
    def mousePressEvent(self, e):
        ...
        if (e.button() == Qt.MouseButton.LeftButton
                and getattr(self._owner, "_cpr_exclude_seeding", False)):
            self._owner._cpr_seed_exclude(
...
class _Pane:
    ...
    def __init__(self, canvas: _PaneCanvas):
        ...
        # Wall-thickness HEAT MAP: a separate scalar (mm) volume resliced on the
        # same plane and coloured blue→red. Its own channel so it never disturbs
        # the blood/region mask above; drawn just over it.
        self.reslice_thick = vtkImageReslice()
        self.reslice_thick.SetInputData(_placeholder_image())
        self.reslice_thick.SetOutputDimensionality(2)
        self.reslice_thick.SetInterpolationModeToNearestNeighbor()
...
class CTViewer(CPRMixin, AbstractViewer):
    ...
    def __init__(self, parent=None):
        ...
        # Chamber-exclude for the lumen snap: seed clicks flood a bright cavity
        # (LV/RV) the snap must ignore. List of (comp, bbox) from chamber_mask.
        self._cpr_exclude = []
        ...
        self._lv = None
        self._lv_view_free = False   # Epi領域表示 inspect state: lock lifted, red on
        ...
        #: "unsaved data" alert ONLY when the current content matches EXACTLY —
        #: a content check (not an event flag), so no edit can be missed.
        #: _lv_saved_sig: per-pass ("endo"/"epi") contour signature;
        #: _lv_valve_saved_sig: per-valve ("mitral"/"aortic") plane signature.
        self._lv_saved_sig = {}
        ...
        # LVD (LV diameter): the MV-leaflet-tip level is set MANUALLY. None until
        # set; _lvv_lvd_shown toggles the on-image line; _lvv_lvd_pending waits
        # for the user to place the level after the first button press.
        self._lvv_lvd_level = None       # along-axis mm from apex, or None
    ...
    def _build_cpr_bar(self) -> QWidget:
        ...
        # 腔除外: arm a seed click inside a bright chamber (LV/RV) → flood-fill it
        # (HU 200–1000) and exclude that region from the lumen snap.
    ...
    def _coronary_sync_ui(self) -> None:
        ...
        if getattr(self, "_cpr_draw_btn", None) is not None:
            ...
            self._cpr_draw_btn.setStyleSheet(
                self._LV_STY["trace"] if pend else self._LV_STY["off"])
    ...
    def _build_lv_bar(self) -> QWidget:
        ...
        # 'Set axis' is retired — Trace auto-captures the axis from the current
        # (auto MV-perpendicular) view. Hidden placeholder keeps references safe.
        ...
        # a sensible default (200–500 HU) and is fine-tuned with the spin arrows.
        # A hidden placeholder keeps old references harmless.
        ...
        # DEFAULT view on entering Blood/Endo, used to fine-tune 下限/上限. Mutually
        # exclusive with LV-Blood表示 (the computed, Epi-clipped region).
        ...
        # the volume on-image. Recomputes when the HU range changed since.
        ...
        # Epi読み込み is retired — the Epi is supplied before/at Blood/Endo entry
        # (trace, or the entry dialog's Load/Create). Hidden placeholder keeps
        # _lvv_epi_load_click references harmless.
        self._lvv_epi_load_btn = FitButton(t("Epi読み込み"))
        self._lvv_epi_load_btn.setVisible(False)
        ...
        # LVD表示: show/hide the LV-diameter measurement. The LVD plane (MV
        # leaflet-tip level) is set MANUALLY — first press prompts to set it on
        ...
        # Auto-Endo 係数: the papillary/trabecula BRIDGING radius (close_mm) that
        # shapes the auto Endo — larger = smoother (compacted-wall / trabeculae
        # INCLUDED in the cavity), smaller = follows the blood pool's indents.
        ...
        self._lvv_close_spin.setToolTip(
            t("Auto-Endo の肉柱/乳頭筋の凹凸を橋渡しする量 (close_mm): 大きいほど"
              "滑らか＝緻密層寄り(肉柱をcavityに含める)、小さいほど血流に忠実"))
        ...
        # Row-2 order (spec): 方式 (method) then 膨らみ (the context-sensitive
        # close/bulge knob, relabelled by _lvv_update_close_ui).
        ...
        # Fit MV: after the MV plane is updated, converge the Epi border to it —
        # delete points on the left-atrium (anti-apex) side of the MV plane and
        # snap the two basal end-points onto the MV plane line in each section.
        ...
        # Blood/Endo row 3 (guided order): Save, Load, Exit, All-Blood, LV-Blood,
        # Epi-Border, Endo-Border(Auto), Endo-Border(Manual). No Clear (per spec).
        # The display toggles (All-Blood … Endo-Border(Manual)) are created up in
        # the row-2 group block above; they are added to THIS row here.
        self._lv_grp_r2_blood = QWidget()
        r2b = QHBoxLayout(self._lv_grp_r2_blood)
        r2b.setContentsMargins(0, 0, 0, 0); r2b.setSpacing(4)
        # Calc Vol is retired — LV-Blood (row 2) computes and shows the volume
        # on-image. Hidden placeholders keep old references harmless.
    ...
    def _lv_pass_sig(self, pas):
        """Content signature of ONE contour pass (its axis + apex + border
        planes) — mirrors exactly the fields _lv_save writes for that pass, so
        'saved' can be judged by CONTENT (no edit path can be missed). None when
        that pass has no border."""
        if self._lv is None:
            return None
        m = self._lv["model"]
    ...
    def _lv_contour_unsaved(self) -> bool:
        """True if the contour (Epi/Endo) model holds a border whose CONTENT does
        not match its last Save/Load — checked per pass by signature, so ANY
        edit (trace, SAX/vertex drag, apex move) is caught regardless of path.
        False only when every present border exactly matches what was saved."""
        if self._lv is None:
            return False
        m = self._lv["model"]
        ...
        for pas in ("endo", "epi"):
            ...
            if cur is None or cur != saved.get(pas):
                return True                    # this pass is unsaved / changed
        return False

    def _lv_valve_sig(self, which):
        """Content signature of a valve plane (centre, normal, radius). None when
        the valve is unset. Tiny + fully captured, so 'saved' is exact."""
        ...
        try:
            ...
            return hashlib.md5(
                json.dumps(payload).encode("utf-8")).hexdigest()
        except Exception:                                # noqa: BLE001
            return None

    def _lv_has_fresh_ellipse(self) -> bool:
        """A drawn Measure→Ellipse not yet Confirmed/tagged — Exiting the valve
        step would discard it, so it must always trigger the confirm."""
        for k in ("A", "B"):
    ...
    def _lv_submode_exit(self) -> None:
        ...
        # Skip the "unsaved data" alert ONLY when we can PROVE nothing would be
        # lost: for a contour pass, when its content matches the last Save/Load
        # exactly (content check — no edit can be missed). Blood ALWAYS confirms
        # (its content signature is not guaranteed complete — safety first).
        if sm == "blood":
            need_warn = True
        else:
    ...
    def _lv_update_submode_ui(self) -> None:
        ...
        # and grey ALL while an Epi/Blood sub-mode is active — the SAME grey-out
        # as Epi/Blood when they have no data; once data is set the button shows
        # its own colour (see _lv_soft_gray + _lv_update_valve_buttons).
        ...
        # Apex selector: selectable from the START (like MV/AoV) — actionable at
        # the selector and during its own step; MV is only *desirable* (warned in
        # _lv_enter_apex). Greyed while editing a valve or in a sub-mode.
    ...
    def _lvv_sync(self) -> None:
        ...
        # are set — HIDE them. Only when no common valves exist does Blood fall
        # back to capturing its own.
        ...
        ready = on and apex_done and valves_ok and (self._lvv_epi_surf is not None)
        ...
        if getattr(self, "_lvv_lvd_btn", None) is not None:
            # LVD needs the Endo mask; available once Auto-Endo can run.
        ...
        self._lvv_epi_load_btn.setVisible(False)   # retired (Epi supplied earlier)
    ...
    def _lvv_confirm_apex(self) -> None:
        ...
        if self._lv_valves_ready():
            lvv["step"] = "ready"
            self._lvv_prompt(
                t("Apex set. Adjust the blood HU range (下限/上限, default "
                  "200–500) using the 全域HU preview, then press 'LV-Blood表示' "
                  "to compute the LV blood inside the Epi surface (水色)."))
        else:
            lvv["step"] = "mv"
            self._lvv_prompt(
    ...
    def _lvv_lvd_reset(self) -> None:
        """Right-click LVD表示 → clear the level and re-enter the set flow."""
        if self._lvv is None:
            return
        self._lvv_lvd_level = None
        self._lvv_lvd_shown = False
    ...
    def _lvv_style_lvd_btn(self) -> None:
        """LVD表示 button: plain when no level set (or 'set me' pending);
        solid gold when set AND shown; gold outline when set but hidden."""
    ...
    def _lvv_update_highlight(self) -> None:
        ...
        the instant 全域HU preview. Shown whenever Blood/Endo is active and the
        LV-Blood (computed) region is NOT being shown (they are exclusive)."""
        on = (self._lvv is not None and getattr(self, "_lvv_hl_on", True)
              and not getattr(self, "_lvv_mask_on", False))
        if on:
    ...
    def _lvv_toggle_highlight(self, *args) -> None:
        # 全域HU表示 and LV-Blood表示 are exclusive: turning the tint ON hides the
        # computed LV-Blood region (and vice versa in _lvv_toggle_blood).
        self._lvv_hl_on = self._lvv_hl_btn.isChecked()
        if self._lvv_hl_on and getattr(self, "_lvv_mask_on", False):
            self._lvv_mask_on = False
            self._lvv_mask_btn.setChecked(False)
            self._lvv_style_toggle(self._lvv_mask_btn, "#40e0ff", "black")
    ...
    def _lvv_toggle_blood(self, *args) -> None:
        ...
        turning ON, compute it for the current HU range if stale/absent (a few
        seconds, off-thread); the 全域HU tint is hidden while it is shown. Turning
        OFF restores the tint."""
        if not self._lvv_mask_btn.isChecked():
            # OFF → hide the 水色 region, restore the instant tint.
            self._lvv_mask_on = False
            self._lvv_style_toggle(self._lvv_mask_btn, "#40e0ff", "black")
            self._lvv_hl_on = True
            self._lvv_hl_btn.setChecked(True)
        ...
        if have:
            ...
            self._lvv_update_highlight()
            return
        # Stale/absent → recompute (the button re-checks itself on success).
        self._lvv_mask_btn.setChecked(False)
        self._lvv_calc()

    def _lvv_signature(self):
        """Hashable snapshot of the inputs that define the LV-Blood region: apex,
        AoV & MV planes (centre + normal) and the HU range read LIVE from the
        spins (the seed is now auto = largest component, so it is not part of the
        signature). Stored at each successful compute (lvv['calc_sig']) and
        compared on the next LV-Blood表示 press — equal → re-show without
        recompute; different → recompute. Returns None if incomplete."""
        lvv = self._lvv
        if lvv is None:
            return None
    ...
    def _lvv_lv_diameter_mm(self):
        ...
        if (comp is None or bbox is None or ax is None or self._dims is None
                or level is None):
            self._lvv_diam_pts = None
            return None                              # LVD is manual: no level → none
        ...
        v = None
        pts = None
        # LVD = the max Endo chord at the MANUALLY-set MV-leaflet-tip along-axis
        # level (perpendicular to the LV long axis).
        try:
            from multi_dicomviewer.core.lv_compact import max_perp_diameter
    ...
    def _lvv_show_diameter(self, render=True) -> None:
        ...
        on = (self._lvv is not None and self._lv is None and pts is not None
              ...
              and getattr(self, "_lvv_lvd_shown", False))
        half = float(getattr(self, "_half", 100.0))
    ...
    def _lvv_update_mask(self) -> None:
        ...
        rgb = getattr(self, "_lvv_mask_rgb", (1.0, 0.25, 0.25))
        for k in ("A", "B"):
            self.pane[k].colors_mask.SetLookupTable(
                _lvv_mask_lut(on, rgb=rgb, alpha=alpha))
    ...
    def _lv_soft_gray(self, btn, actionable, has_data) -> None:
        ...
        - has_data: keep ENABLED (so right-click hide/show works) and mark
          non-actionable so its LEFT-click no-ops; _lv_update_valve_buttons
          paints it in the data colour.
        - no data: enable only when actionable (normal button); when not
          actionable disable it → standard grey-out (same _BTN_DIS as Epi/Blood).
          Nothing to hide/show, so right-click is not needed."""
        if btn is None:
            return
        act = bool(actionable)
    ...
    def _lv_exit_valve(self) -> None:
        ...
        plane stays in memory (Confirm set it); only an un-confirmed draw ellipse
        is dropped. The alert is skipped ONLY when it is safe: the plane matches
        the last Save/Load AND there is no un-confirmed ellipse to lose."""
        from PyQt6.QtWidgets import QMessageBox
        ...
        # Warn unless: nothing to lose (no un-confirmed ellipse) AND either the
        # plane is unset or it exactly matches what was saved.
    ...
    def _lv_on_valve_changed(self, which) -> None:
        ...
        caches (Epi border mask, Auto-Endo mask, 壁厚, LVL) so a redisplay/
        recompute uses the NEW plane while REUSING the trace. In contour LV,
        re-snap the traced border's basal ends onto the new MV so no full
        re-trace is needed."""
        try:
            self._lvv_invalidate_epi_disp()
        except Exception:                                # noqa: BLE001
            pass
        try:
            self._lvv_invalidate_auto_endo()
        ...
        if (which == "mitral" and self._lv is not None
            ...
            for _pas in ("endo", "epi"):
                try:
                    ...
                    pass
        # Refresh the on-image Epi/Endo overlays for the new plane.
        try:
            if self._lvv is not None:
    ...
    def _lv_update_valve_buttons(self) -> None:
        """MV / AoV buttons: plain when unset; solid colour when set, shown AND
        selectable; a FAINT tint when set+shown but not selected (soft-off); a
        coloured outline when set but hidden."""
        ...
        # Apex selector: same convention (red).
        ...
        # enabled/actionable, standard grey when soft-disabled — same as
        # Epi/Blood); a SET selector always shows its data colour above.
        ...
        try:
            ...
            QMessageBox.information(
                ...
                t("Loaded the {w} plane. Epi/Endo reuse the existing trace with "
                  "the new basal cut — re-run LV-Blood / Auto-Endo (or Calc Vol) "
                  "to refresh the volumes.").format(
                    w="MV" if which == "mitral" else "AoV"))
        except Exception as exc:                        # noqa: BLE001
            import traceback
        ...
        class _ThickWorker(QThread):
            def run(self_) -> None:
                try:
                    ...
                    # Place the endo mask (its own bbox) into the epi sub-box.
    ...
    def _lvv_calc_finish(self, res, dlg) -> None:
        ...
        try:
            ...
            # Build the LV-Blood region mask volume (0/1) for the 水色 overlay.
            comp = res.get("comp")
            if comp is not None:
                z0, z1, y0, y1, x0, x1 = res["bbox"]
                full = np.zeros(self._vol.shape, np.float32)
                full[z0:z1, y0:y1, x0:x1][np.asarray(comp, bool)] = 1.0
                # Keep the blood mask (bbox sub-volume) + apex so Auto-Endo can
                ...
                self._lvv_mask_rgb = (0.25, 0.75, 1.0)   # LV-Blood = 水色
    ...
    def _lvv_do_save(self) -> None:
        ...
        # The Epi is NOT duplicated here — it stays single-source in EpiLv.json
        # (its region mask). Offline function analysis reads EpiLv (Epi) + BldLv
        # (Endo/blood) together, so the two can never drift out of sync.
    ...
    def _lv_sync_buttons(self) -> None:
        ...
        if getattr(self, "_lv_fit_mv_btn", None) is not None:
            self._lv_fit_mv_btn.setEnabled(
                data_ok and self._lv_valves.get("mitral") is not None)
    ...
    def _measure_to_history(self, key, m):
        """Build a Measurement history record for a committed/edited/pasted
        general measure (LV borders/regions are not history figures → None)."""
        if m is None or m.get("_lv") is not None or m.get("_lvv") is not None:
            return None
        if m.get("id") is None:
            return None
        return Measurement(
            kind=self._JP.get(m.get("type"), m.get("type", "")),
            points=[tuple(q) for q in m.get("pts", [])],
            text=self._metrics_text(key, m),
            mid=int(m["id"]))
    ...
    def _copy_measure(self, key, mi) -> None:
        """Right-click 'Copy': stash a deep copy of this measure on the clipboard
        for Ctrl+V paste (as a new figure)."""
    ...
    def _paste_measure(self) -> None:
        ...
        pane, offset slightly so it doesn't sit exactly on the original. Its
        result shows as the on-image overlay (and always in Measure History; in
        the Blood/Endo limited-result view the on-image metrics stay hidden, so
        the history is where the number appears)."""
        import copy as _copy
        clip = getattr(self, "_meas_clipboard", None)
        if not clip or self._image is None:
            return
        key = getattr(self, "_active_pane", "A") or "A"
        m = _copy.deepcopy(clip)
        self._meas_seq = getattr(self, "_meas_seq", 0) + 1
        m["id"] = self._meas_seq
        for k in ("_lv", "_lvv", "center_angle"):     # a paste is a free measure
            m.pop(k, None)
        m["hidden"] = False
        ...
        if m.get("pts3d"):
            ...
            m["pts3d"] = [tuple(np.asarray(P, float) + off3d)
                          for P in m["pts3d"]]
    ...
    def lv_active(self) -> bool:
        ...
        Blood / Auto-Endo / region masks. The shell must NOT auto-demote such a
        pane to a memory-saving 'still': demoting frees the volume and a later
        promote reloads (dropping this state) — the whole point of keeping it
        live is that re-displaying it needs NO reload or recompute."""
        ...
        for attr in ("_lvv_epi_surf", "_lvv_blood_comp", "_lv_endo_mask_comp",
                     "_lv_region_comp", "_lvv_epi_disp_comp"):
    ...
    def _lv_enter_align(self) -> None:
        ...
        orients the view; Trace then captures the axis. Trace/analysis controls
    ...
    def _lv_toggle_sax(self) -> None:
        ...
        from PyQt6.QtWidgets import QMessageBox
        self._lv_region_reset()                      # SAX has its own view state
        lv = self._lv
    ...
    def _lv_sax_apply_default_scale(self) -> None:
        ...
        (short-axis) pane: the Epi border's max radius = 30% of the pane WIDTH."""
    ...
    def _lv_sax_short_scale(self, ax, along0):
        ...
        border's MAX radius (from the axis centre) = 45% of the pane WIDTH (Epi
        diameter ≈ 90% of the frame, fits inside). ParallelScale is half the pane
        HEIGHT in mm, so a radius that must land at 0.30·width_px needs
        ps = rmax·height_px / (0.60·width_px). None when there is no Epi crossing
    ...
    def _lv_snap_base_to_mv(self, pas, guard_mm: float = 20.0,
        ...
        # basal end more than 20 mm from the plane (the reported "far end doesn't
        # converge" bug); the apex end (along≈0) is still never caught.
        apex = np.asarray(ax.apex, float)
        axis = np.asarray(ax.axis, float)
        axis = axis / (float(np.linalg.norm(axis)) or 1.0)
    ...
    def _lv_fit_mv(self) -> None:
        ...
        points onto the MV line at 5 mm radial from the LV long axis, each on its
        ...
        along_mv = float((c - apex) @ axis)          # base level along the axis
        thr = 0.4 * along_mv if along_mv > 1.0 else -1.0
        sa = float(np.sign((apex - c) @ n)) or 1.0   # apex side of the MV plane
        eps = 1e-6

        def _snap_to_mv_line(P, phi):
            """Place the basal endpoint 5 mm RADIAL from the LV long axis, on P's
            own wall side, at the MV-centre (base) level: c + 5·s·e_s, where e_s is
            the meridian's unit radial direction (⟂ the axis) and c (the MV centre)
            is on the axis. Since e_s ⟂ axis, this is exactly 5 mm off-axis with NO
            move along the axis. (Constraining it to a TILTED MV *line* instead
            shot the point far up/down the axis — the reported 'line goes straight
            up'.)"""
            e_s = np.asarray(ax.meridian_dir(phi), float)   # radial dir (⟂ axis)
            ne = float(np.linalg.norm(e_s))
            if ne < eps:
                return P
            e_s = e_s / ne
        ...
        for phi, arr in list(store.items()):
            ...
            if len(P) < 2:
                continue
            # 1) Drop points on the LEFT-ATRIUM side of the MV plane (the side
            #    opposite the apex): keep the apex-side points (and on-plane).
            keep = ((P - c) @ n) * sa >= -1e-3
            Pk = P[keep]
            if len(Pk) < 2:
                continue                             # nothing sensible left
            # 2) Snap the two BASAL end points onto the MV line (apex end guarded
            #    by the along-axis threshold, so a half-trace's apex isn't moved).
        ...
        if not changed:
            ...
            return
        self._lv_invalidate_volume()                 # geometry changed
        self._lv_rebuild_measures()
        self._lv_show_plane()
        self._lv_record_geom(before)                 # one Ctrl+Z step
        self._lv_dirty = True
    ...
    def _lv_show_measured_mask(self, mask) -> None:
        ...
        sx, sy, sz = self._dims
        self._lvv_mask_vol = numpy_to_vtk_image(full, sx, sy, sz)
        self._lvv_mask_rgb = (1.0, 0.25, 0.25)  # Endo/Epi region = red
    ...
    def _snap_to_lumen(self, P, n, reach=8.0, floor_hu=150.0,
        ...
        *floor_hu* within reach, or the volume isn't available."""
        ...
        bright = hu >= thr
        # Contiguous bright runs; keep only VESSEL-SIZED ones (width along the
        # normal ≤ max_lumen_mm — a coronary lumen is a few mm; a cavity spans
        # much more), then choose the one nearest d=0 (the click).
        best = None
        i = 0
        N = len(ds)
        while i < N:
        ...
        # cross-section plane at the candidate point — reject if it's wider than
        # a vessel there (a chamber spreads far in-plane even at a grazing hit).
    ...
    def _bright_too_wide_inplane(self, Pc, n, thr, max_mm):
        ...
        across any in-plane axis ⟂ *n* — i.e. a chamber/great-vessel cavity, not
        a coronary lumen. Casts 12 rays (6 axes) in the plane and checks each
        axis's diameter (extent one way + the other)."""
        import math
        n = np.asarray(n, float)
    ...
    def _excl_apply(self, hu, origin, dirn, offs):
        """Set samples that fall inside a chamber-exclude mask below floor, so
        the lumen snap treats them as non-bright."""
    ...
    def _cpr_seed_exclude(self, which, sx, sy):
        ...
        snap exclusion set. Armed by the 腔除外 button; disarms after one seed."""
        from PyQt6.QtWidgets import QMessageBox
        ...
        if self._vol is None:
            return
        wx, wy = self._disp_to_world(which, sx, sy)
        P = self._out_to_world3d(which, wx, wy)          # world-mm on the plane
        ...
        vox_ml = float(np.prod(self._dims)) / 1000.0
        vol_ml = int(comp.sum()) * vox_ml
        if vol_ml > 300.0:                               # likely a leak
            QMessageBox.warning(self, t("腔除外"), t(
                "抽出領域が大きすぎます ({v:.0f} mL)。大動脈や漏れの可能性が"
                "あります。「除外クリア」してHU下限を上げるか別の点で試して"
                "ください。", v=vol_ml))
        self._refresh()

    def _cpr_clear_exclude(self):
        """Drop all chamber-exclude masks."""
        self._cpr_exclude = []
        self._refresh()
    ...
    def _exit_cpr(self, keep_view=False):
        ...
        *keep_view* restores the MPR view that was showing when this short-axis
        was built (so re-Draw traces on the SAME image), instead of resetting to
        a default MPR."""
    ...
    def _refresh(self, reset_cam=False, only=None):
        ...
        for key in ("A", "B"):
            ...
            if getattr(self, "_lvv_thick_vol", None) is not None:
                ...
                p.reslice_thick.SetOutputSpacing(spacing, spacing, base_step)
                p.reslice_thick.SetOutputOrigin(fx - box_u, fy - box_v, 0.0)
                p.reslice_thick.SetOutputExtent(0, nu - 1, 0, nv - 1, 0, 0)
                if hasattr(p.reslice_thick, "SetSlabNumberOfSlices"):
                    p.reslice_thick.SetSlabNumberOfSlices(1)
                p.reslice_thick.Modified()
            p.colors.SetLookupTable(self._lut())
            p.colors.Modified()
    ...
    def _lv_cross_suppressed(self) -> bool:
        ...
        Epi領域表示 (inspect/free-view) also un-suppresses it."""
        if getattr(self, "_lv_view_free", False):
            return False
    ...
    def _rotate_companion_by(self, which, d_deg) -> None:
        ...
        # The crossing's CURRENT in-plane offset from the companion reslice centre
        # (old frame). Keep it so the crossing holds its SCREEN position — rotate
        # the image AROUND the crossing instead of snapping it to the pane centre
        # (which jumped a user-moved crossing back to the middle).
        P = np.asarray(self._center, float)
