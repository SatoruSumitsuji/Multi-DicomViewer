...
class CTViewer(CPRMixin, AbstractViewer):
    ...
    def _set_mode(self, mode, reset_cam=False):
        ...
        if mode not in ("3D", "2D") or self._vol is None:
            return
        # A Plane/2D/3D switch leaves short-axis (it repurposes pane A) and
    ...
    def _lv_cosync_axis(self):
        ...
        if ax is not None:
            ...
            # Fallback: LV long axis = common apex → MV-centre (works without a
            # built Epi surface, so the level link 按分/mm is available whenever
            # MV + apex are set).
        ...
        if nrm < 1e-9:
            return None
        axis = axis / nrm
        return apex, axis, 0.0, (length or 90.0)
    ...
    def _lv_valve_confirm(self) -> None:
        ...
        Re-pressing Confirm updates it; it never toggles the ring's visibility
    ...
    def _lv_show_sax_both(self) -> None:
        ...
        if first:
            ...
            # only showed after a user nudge. Re-applying next event-loop turn
            # (correct sizes, scale locked) makes it show at once.
    ...
    def _lv_sax_refit_default(self) -> None:
        ...
        settled the pane sizes, then render — so entry shows the default
        size/place with no user nudge. No-op if SAX was left meanwhile."""
        lv = getattr(self, "_lv", None)
        if (lv is None or lv.get("sax") is None or self._vol is None
    ...
    def _lv_sax_short_scale(self, ax, along0):
        ...
        at 0.30·width_px needs ps = rmax·height_px / (0.60·width_px). None when
    ...
    def _lv_snap_base_to_mv(self, pas, guard_mm: float = 20.0,
        ...
        for phi, arr in list(store.items()):
            ...
            # New basal terminal = 5 mm RADIAL from the LV long axis at the
            # MV-centre (base) level, on the end's own wall side: c + 5·s·e_s —
            # the SAME convergence 'Fit MV' uses. PREPENDED (first) / APPENDED
            # (last) so the traced end survives.
    ...
    def capture_view_state(self) -> dict:
        """Full display state for the Case Presentation tool (slice + MPR camera
        + W/L + CPR). Reuses the undo/redo snapshot. Mirrors the VTK viewer."""
        try:
    ...
    def _ct_rehydrate(self, st: dict) -> dict:
        ...
        if isinstance(s.get("pan"), dict):
            s["pan"] = {k: (np.asarray(v, float) if isinstance(v, list) else v)
                        for k, v in s["pan"].items()}
    ...
    def _build_cpr_bar(self) -> QWidget:
        ...
        # the display state — follows an in-place Measure edit of the centreline
        # without a Save→Exit→Load round-trip.
        ...
        # the layout (so the buttons keep their natural width); its CHILDREN are
        # shown only once a CPR is built.
    ...
    def _coronary_sync_ui(self) -> None:
        ...
        for w in getattr(self, "_cpr_scrub_widgets", ()):
            w.setVisible(cpr)
    ...
    def _toggle_coronary_mpr(self):
        """Coronary: CPR button — enter (or leave) coronary MPR mode, which shows
        the Draw / Load / Save / Exit row below."""
        if self._vol is None:
            self._coronary_sync_ui()
            return
    ...
    def _coronary_draw(self):
        ...
        so a left-drag traces the vessel centreline."""
        if self._vol is None or self._mode != "3D":
            self._coronary_sync_ui()
            return
        self._coronary_mode = True
    ...
    def _coronary_exit(self):
        """Exit button — leave short-axis (if built) and coronary MPR mode,
    ...
    def _cpr_drop_source(self):
        ...
        source measure + any _cpr_src-tagged auto-trace)."""
        c = self._cpr
        if c is not None:
